...
bool ScriptLoader::ProcessScriptElement(nsIScriptElement* aElement,
  ...
  } else if (aElement->GetScriptIsMersey()) {
    scriptKind = ScriptKind::eMersey;
  } else {
...
bool ScriptLoader::ProcessExternalScript(nsIScriptElement* aElement,
  ...
  // Mersey fork: external Mersey scripts arrive with the module loader — the
  // contract (msy_context_scan_imports / msy_context_run_graph) is already in
  // the ABI, and the host is what fetches. Inline only, for now.
  if (aScriptKind == ScriptKind::eMersey) {
    NS_DispatchToCurrentThread(
        NewRunnableMethod("nsIScriptElement::FireErrorEvent", aElement,
                          &nsIScriptElement::FireErrorEvent));
    nsContentUtils::ReportToConsoleNonLocalized(
        u"External Mersey scripts are not yet supported; inline only."_ns,
...
bool ScriptLoader::ProcessInlineScript(nsIScriptElement* aElement,
  ...
  // Mersey fork: an inline Mersey script runs here, on the Mersey engine, the
  // way an inline speculation-rules block is consumed at this point. It is not
  // JavaScript, so no ScriptLoadRequest is created and SpiderMonkey is never
  // involved. Note the position: *after* the CSP check, which is where it
  // belongs — a different language is not a different security policy.
  //
  // Diagnostics and runtime errors reach the console through the engine's host
  // table (dom/mersey/MerseyScriptRunner.cpp).
  if (aScriptKind == ScriptKind::eMersey) {
    // A trusted inline script arrives with a void source text — Gecko fetches
    // it lazily (bug 1376651). Do that now, exactly as the speculation-rules
    // path does, rather than trusting aSourceText.
    nsAutoString source;
    aElement->GetScriptText(source);
    MerseyScriptRunner::For(mDocument)->Run(source);
    return false;
  }
