...
bool ScriptElement::MaybeProcessScript(const nsAString& aSourceText) {
  ...
  if (!type.IsEmpty()) {
    if (!nsContentUtils::IsJavascriptMIMEType(type) &&
        ...
        // Mersey fork: text/mersey is an executable script type — the Mersey
        // engine's, not JavaScript's. Without this the parser treats it as a
        // data block and never processes the element.
        !type.LowerCaseEqualsASCII("text/mersey") &&
