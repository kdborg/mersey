...
partial namespace ChromeUtils {
  ...
  */
  ...
  /**
   * Mersey fork: run one DevTools-console REPL turn against `window`'s Mersey
   * engine. The session is one growing, always-typechecked module — the
   * language has no `eval`, so a turn is the ordinary compile pipeline over an
   * appended fragment, not a separate interpreter mode.
   *
   * Returns the echo of a trailing bare expression; "" when the turn only
   * declared something; "!"-prefixed diagnostics when the turn was REJECTED
   * (it never ran); "runtime error:"-prefixed when it threw.
   *
   * No binding is shared with the JS realm — the engine holds integer handles
   * and reaches the page only through its host table, so neither language can
   * name the other's variables.
   */
  [ChromeOnly, Throws]
  DOMString merseyReplTurn(WindowProxy window, DOMString source);

  /**
   * Mersey fork: the names the window's Mersey session can see, as a JSON
   * array — what the console offers for completion in Mersey mode.
   *
   * This is Mersey's view, NOT the JS realm's: the session's own top-level
   * bindings plus its std:console prelude, and deliberately no
   * window/document. No JS global can appear here.
   */
  [ChromeOnly, Throws]
  DOMString merseyCompletionsJson(WindowProxy window);

  /**
   * Mersey fork: the window's executed Mersey sources, as a JSON array of
   * {source} — what the DevTools Mersey debugger lists and sets breakpoints
   * in (engine line numbers; no document offsets).
   */
  [ChromeOnly, Throws]
  DOMString merseyDebugScripts(WindowProxy window);

  /**
   * REPLACES the engine breakpoint set (engine 1-based lines). Setting
   * breakpoints installs the debug hook itself. While the engine is paused
   * it blocks its callout and spins the event loop; observer notifications
   * "mersey-debugger-paused"/"mersey-debugger-resumed" (JSON payload with
   * the inner window id) tell DevTools what happened.
   */
  [ChromeOnly, Throws]
  undefined merseyDebugSetBreakpoints(WindowProxy window,
                                      sequence<unsigned long> lines);

  /** "pause" | "resume" | "stepOver" | "stepInto" | "stepOut" | "disable". */
  [ChromeOnly, Throws]
  undefined merseyDebugAction(WindowProxy window, DOMString action);
