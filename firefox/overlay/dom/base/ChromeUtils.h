...
class ChromeUtils {
 ...
 public:
  ...
  // Mersey fork: one console REPL turn against a window's Mersey engine.
  static void MerseyReplTurn(GlobalObject& aGlobal, WindowProxyHolder& aWindow,
                             const nsAString& aSource, nsAString& aRetval,
                             ErrorResult& aRv);
  static void MerseyCompletionsJson(GlobalObject& aGlobal,
                                    WindowProxyHolder& aWindow,
                                    nsAString& aRetval, ErrorResult& aRv);
  static void MerseyDebugScripts(GlobalObject& aGlobal,
                                 WindowProxyHolder& aWindow, nsAString& aRetval,
                                 ErrorResult& aRv);
  static void MerseyDebugSetBreakpoints(GlobalObject& aGlobal,
                                        WindowProxyHolder& aWindow,
                                        const Sequence<uint32_t>& aLines,
                                        ErrorResult& aRv);
  static void MerseyDebugAction(GlobalObject& aGlobal,
                                WindowProxyHolder& aWindow,
                                const nsAString& aAction, ErrorResult& aRv);
