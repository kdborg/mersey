...
/* static */
/* static */
void ChromeUtils::MerseyDebugScripts(GlobalObject& aGlobal,
                                     WindowProxyHolder& aWindow,
                                     nsAString& aRetval, ErrorResult& aRv) {
  aRetval.AssignLiteral(u"[]");
  nsIDocShell* docShell = aWindow.get() ? aWindow.get()->GetDocShell() : nullptr;
  RefPtr<Document> doc = docShell ? docShell->GetDocument() : nullptr;
  MerseyScriptRunner* runner = doc ? MerseyScriptRunner::For(doc) : nullptr;
  if (!runner) {
    aRv.ThrowInvalidStateError("no Mersey engine for window");
    return;
  }
  nsAutoCString out;
  runner->DebugScriptsJson(out);
  CopyUTF8toUTF16(out, aRetval);
}

/* static */
void ChromeUtils::MerseyDebugSetBreakpoints(GlobalObject& aGlobal,
                                            WindowProxyHolder& aWindow,
                                            const Sequence<uint32_t>& aLines,
                                            ErrorResult& aRv) {
  nsIDocShell* docShell = aWindow.get() ? aWindow.get()->GetDocShell() : nullptr;
  RefPtr<Document> doc = docShell ? docShell->GetDocument() : nullptr;
  MerseyScriptRunner* runner = doc ? MerseyScriptRunner::For(doc) : nullptr;
  ...
  lines.AppendElements(aLines.Elements(), aLines.Length());
  runner->DebugSetBreakpoints(lines);
}

/* static */
void ChromeUtils::MerseyDebugAction(GlobalObject& aGlobal,
                                    WindowProxyHolder& aWindow,
  ...
  runner->DebugAction(NS_ConvertUTF16toUTF8(aAction));
}

/* static */
void ChromeUtils::MerseyCompletionsJson(GlobalObject& aGlobal,
                                        WindowProxyHolder& aWindow,
                                        nsAString& aRetval, ErrorResult& aRv) {
  aRetval.AssignLiteral("[]");

  nsIDocShell* docShell = aWindow.get() ? aWindow.get()->GetDocShell() : nullptr;
  if (!docShell) {
    aRv.ThrowInvalidStateError("no docshell for window");
    return;
  }
  RefPtr<Document> doc = docShell->GetDocument();
  if (!doc) {
    aRv.ThrowInvalidStateError("no document for window");
    return;
  }
  MerseyScriptRunner* runner = MerseyScriptRunner::For(doc);
  if (!runner) {
    aRv.ThrowInvalidStateError("no Mersey engine for document");
    return;
  }

  nsAutoCString out;
  runner->ReplCompletions(out);
  CopyUTF8toUTF16(out, aRetval);
}

/* static */
void ChromeUtils::MerseyReplTurn(GlobalObject& aGlobal,
                                 WindowProxyHolder& aWindow,
                                 const nsAString& aSource, nsAString& aRetval,
                                 ErrorResult& aRv) {
  aRetval.Truncate();
  ...
  // Creates the engine context on first use, so the console works in Mersey
  // mode on any page — not only ones carrying a <script type="text/mersey">.
  MerseyScriptRunner* runner = MerseyScriptRunner::For(doc);
  if (!runner) {
    aRv.ThrowInvalidStateError("no Mersey engine for document");
    return;
  ...
  nsAutoCString out;
  runner->ReplTurn(NS_ConvertUTF16toUTF8(aSource), out);
  CopyUTF8toUTF16(out, aRetval);
}
