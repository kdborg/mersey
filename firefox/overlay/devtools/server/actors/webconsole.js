...
class WebConsoleActor extends Actor {
   ...
   */
  /**
   * Mersey fork: one console turn in the page's Mersey engine.
   *
   * The engine returns a display string, not a JS value, so there is nothing
   * to grip: an echo comes back as a plain string result, and both failure
   * shapes ("!" = rejected before running, "runtime error:" = threw) come back
   * as an exception the console renders in red.
   */
  _evaluateMersey(request) {
    const input = request.text;
    const window = this.global;
    try {
      const reply = ChromeUtils.merseyReplTurn(window, input);
      if (reply.startsWith("!")) {
        const message = reply.slice(1);
        return {
          input,
          result: null,
          exception: message,
      ...
      if (reply.startsWith("runtime error:")) {
        return {
          input,
          result: null,
          exception: reply,
          exceptionMessage: reply,
        };
      }
      // A turn that only declared something echoes nothing.
      return { input, result: reply === "" ? { type: "undefined" } : reply };
    } catch (e) {
  ...
  async evaluateJSAsync(request) {
    ...
    DevToolsUtils.executeSoonWithMicroTask(async () => {
      try {
        // Mersey fork: a Mersey turn never reaches the JS evaluator. It runs
        // in the page's own Mersey engine, which shares no bindings with this
        // realm (see mersey/docs/architecture/devtools.md).
        if (request.language === "mersey") {
          const merseyResponse = this._evaluateMersey(request);
          merseyResponse.timestamp = ChromeUtils.dateNow();
          merseyResponse.resultID = resultID;
          merseyResponse.startTime = startTime;
          this.emit("evaluationResult", merseyResponse);
          return;
        }
   ...
   */
  /**
   * Mersey fork: the completion surface in Mersey mode.
   *
   * This is MERSEY's view — the session's own top-level bindings plus its
   * prelude — and never the JS realm's. No JS global, function, or variable
   * appears here, which is the whole point: the two languages share nothing.
   */
  _merseyCompletions(text, cursor) {
    const reqText = text.substr(0, cursor === undefined ? text.length : cursor);
    const match = reqText.match(/[A-Za-z_$][\w$]*$/);
    const prefix = match ? match[0] : "";
    let names = [];
    try {
      names = JSON.parse(ChromeUtils.merseyCompletionsJson(this.global));
    } catch (e) {
      names = [];
    }
  ...
  autocomplete(
    ...
    if (language === "mersey") {
      return this._merseyCompletions(text, cursor);
    }
    let dbgObject = null;
