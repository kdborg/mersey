...
function evaluateExpression(expression, from = "input") {
  return async ({ dispatch, webConsoleUI, hud, commands, getState }) => {
    ...
    const response = await commands.scriptCommand
      .execute(expression, {
        language: getState().ui.evalLanguage,
...
function terminalInputChanged(expression, force = false) {
  return async ({ dispatch, webConsoleUI, hud, commands, getState }) => {
    ...
    // Mersey fork: no eager evaluation in Mersey mode. A turn appends to the
    // session's growing module, so running it as the user types would mutate
    // the very program being written.
    if (getState().ui.evalLanguage === "mersey") {
      return dispatch({
        type: SET_TERMINAL_EAGER_RESULT,
        expression,
        result: null,
