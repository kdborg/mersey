...
class EvaluationContextSelector extends Component {
  static get propTypes() {
    return {
      ...
      // Mersey fork: the console input's language, and how to change it.
      ui: PropTypes.object,
      setEvalLanguage: PropTypes.func,
    };
  }
  ...
  renderMenuItems() {
    ...
    // Mersey fork: the language switch. Mersey is a separate engine with its
    // own bindings, so this is genuinely a second evaluation context — which
    // is why it belongs in this menu rather than a toolbar of its own.
    const currentLanguage = this.props.ui?.evalLanguage || "javascript";
    const languageItems = [
      dom.hr({ role: "menuseparator", key: "mersey-language-separator" }),
    ];
    for (const [value, label] of [
      ...
      languageItems.push(
        MenuItem({
          key: `mersey-language-${value}`,
          className: "menu-item webconsole-evaluation-selector-item",
          type: "checkbox",
          checked: currentLanguage === value,
          label,
          tooltip: `Evaluate console input as ${label}`,
          onClick: () => this.props.setEvalLanguage?.(value),
        })
      );
    }
  ...
  getLabel() {
    ...
    // Mersey fork: surface the language on the button, so the console's mode
    // is visible without opening the menu.
    const languageSuffix =
      this.props.ui?.evalLanguage === "mersey" ? " \u00b7 Mersey" : "";
    if (languageSuffix) {
      const base =
        !selectedTarget ||
...
module.exports = connect(
  state => state,
  dispatch => ({
    setEvalLanguage: language =>
      dispatch(webconsoleActions.setEvalLanguage(language)),
    onContextChange: () => {
      dispatch(
