...
impl BigInt {
    ...
    fn mul_small(&self, m: u32) -> BigInt {
        ...
        BigInt { neg: self.neg, mag: out }.trim()
    }
    ...
    fn divmod_small(&self, d: u32) -> (BigInt, u32) {
        ...
        (BigInt { neg: self.neg, mag: out }.trim(), rem as u32)
    }
    ...
    pub fn add(&self, other: &BigInt) -> BigInt {
        if self.neg == other.neg {
            ...
            match Self::cmp_mag(&self.mag, &other.mag) {
                ...
                Ordering::Greater => {
                    BigInt { neg: self.neg, mag: Self::sub_mag(&self.mag, &other.mag) }.trim()
                }
                Ordering::Less => {
                    BigInt { neg: other.neg, mag: Self::sub_mag(&other.mag, &self.mag) }.trim()
                }
            }
        }
    }
    ...
    pub fn mul(&self, other: &BigInt) -> BigInt {
        ...
        BigInt { neg: self.neg != other.neg, mag: out }.trim()
    }
    ...
    pub fn divmod(&self, other: &BigInt) -> Option<(BigInt, BigInt)> {
        ...
        let mut rem = BigInt { neg: false, mag: self.mag.clone() };
        let mut quot = BigInt::zero();
        for s in (0..=shift).rev() {
            let d = other.shl_bits(s);
            let da = BigInt { neg: false, mag: d.mag.clone() };
            if Self::cmp_mag(&rem.mag, &da.mag) != Ordering::Less {
                rem = BigInt { neg: false, mag: Self::sub_mag(&rem.mag, &da.mag) }.trim();
    ...
    pub fn to_decimal(&self) -> String {
        ...
        let mut n = BigInt { neg: false, mag: self.mag.clone() };
        while !n.is_zero() {
            let (q, r) = n.divmod_small(1_000_000_000);
            digits.push(r);
            n = q;
        }
        let mut s = if self.neg { "-".to_string() } else { String::new() };
...
impl BigDec {
    ...
    pub fn parse(text: &str) -> Option<BigDec> {
        ...
        Some(if scale >= 0 {
            BigDec { coef, scale: u32::try_from(scale).ok()? }
        } else {
            // negative scale: multiply out
            BigDec { coef: coef.mul(&BigInt::pow10((-scale) as u32)), scale: 0 }
        })
    }
    ...
    pub fn add(&self, other: &BigDec) -> BigDec {
        ...
        BigDec { coef: a.add(&b), scale }
    }

    pub fn sub(&self, other: &BigDec) -> BigDec {
        let (a, b, scale) = Self::align(self, other);
        BigDec { coef: a.sub(&b), scale }
    }

    pub fn mul(&self, other: &BigDec) -> BigDec {
        BigDec { coef: self.coef.mul(&other.coef), scale: self.scale + other.scale }
    }
        ...
        for extra in 0..=MAX_EXTRA {
            ...
            if r.is_zero() {
                ...
                return Some(if final_scale >= 0 {
                    BigDec { coef: q, scale: final_scale as u32 }
                } else {
                    BigDec { coef: q.mul(&BigInt::pow10((-final_scale) as u32)), scale: 0 }
                });
            }
    ...
    pub fn to_decimal(&self) -> String {
        let digits = {
            let d = BigInt { neg: false, mag: self.coef.mag.clone() }.to_decimal();
            d
        };
...
mod tests {
    ...
    fn bigdec_basics() {
        ...
        let half = BigDec::parse("1").unwrap().div_exact(&BigDec::parse("8").unwrap()).unwrap();
        assert_eq!(half.to_decimal(), "0.125");
        assert!(BigDec::parse("1").unwrap().div_exact(&BigDec::parse("3").unwrap()).is_none());
