...
for (const [name, rec] of interfaces) {
  ...
  for (const m of allMembers(name)) {
    if (m.type === "attribute" && m.special !== "static") {
      ...
      const variadic = (m.arguments ?? []).some((a) => a.variadic);
      bindCalls.push([
        `${name}.${m.name}`,
        variadic
          ? `(t,a)=>t.${m.name}(...a)`
          : `(t,a)=>t.${m.name}(${argList((m.arguments ?? []).length)})`,
      ]);
      counts.members++;
      counts.bindings = (counts.bindings ?? 0) + 1;
    }
...
// Globals: Window's attributes & operations become ambient globals.
const winMembers = allMembers("Window");
const seenG = new Set(["console"]);
