...
impl ConsoleActor {
    ...
    fn evaluate_js(
        ...
        // A leading `mersey>` evaluates the turn against the page's Mersey engine
        // rather than JS: rewrite it to call the page-global `mersey(source)`
        // (installed by components/script/mersey) and let the ordinary JS eval
        // path carry it. This is what lets a stock DevTools console — which has
        // no Mersey language selector yet — drive Mersey against Servo today.
        // The reply below still echoes `input`, i.e. what the user typed.
        let code = match input.strip_prefix("mersey>") {
            Some(rest) => format!(
                "mersey({})",
                serde_json::to_string(rest.trim_start()).unwrap_or_else(|_| "\"\"".to_owned())
            ),
            None => input.clone(),
        };
