...
NonnullRefPtr<FrameActor> FrameActor::create(DevToolsServer& devtools, String name, WeakPtr<TabActor> tab, WeakPtr<WatcherActor> watcher, WeakPtr<CSSPropertiesActor> css_properties, WeakPtr<ConsoleActor> console, WeakPtr<InspectorActor> inspector, WeakPtr<StyleSheetsActor> style_sheets, WeakPtr<ThreadActor> thread, WeakPtr<AccessibilityActor> accessibility, WeakPtr<MerseyDebuggerActor> mersey_debugger)
{
    return adopt_ref(*new FrameActor(devtools, move(name), move(tab), move(watcher), move(css_properties), move(console), move(inspector), move(style_sheets), move(thread), move(accessibility), move(mersey_debugger)));
}
...
FrameActor::FrameActor(DevToolsServer& devtools, String name, WeakPtr<TabActor> tab, WeakPtr<WatcherActor> watcher, WeakPtr<CSSPropertiesActor> css_properties, WeakPtr<ConsoleActor> console, WeakPtr<InspectorActor> inspector, WeakPtr<StyleSheetsActor> style_sheets, WeakPtr<ThreadActor> thread, WeakPtr<AccessibilityActor> accessibility, WeakPtr<MerseyDebuggerActor> mersey_debugger)
    ...
    , m_mersey_debugger(move(mersey_debugger))
{
...
JsonObject FrameActor::serialize_target() const
{
    ...
    if (auto mersey_debugger = m_mersey_debugger.strong_ref())
        target.set("merseyDebuggerActor"sv, mersey_debugger->name());
