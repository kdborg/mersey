...
FrameActor& WatcherActor::create_frame_target()
{
    ...
    auto& mersey_debugger = devtools().register_actor<MerseyDebuggerActor>(m_tab);

    auto& target = devtools().register_actor<FrameActor>(m_tab, make_weak_ptr<WatcherActor>(), css_properties, console, inspector, style_sheets, thread, accessibility, mersey_debugger);
    m_target = target;
    m_thread = thread;
    return target;
}
