...
// Core->modules hook backing the Mersey engine's localStorage: `core` (where the
// script runner lives) cannot depend on `modules/storage`, so these adapters run
// here and are registered into the core-side bridge at startup.
//
// The StorageArea for a window is stable for its lifetime, but resolving it
// (DOMWindowStorage::localStorage) is not free — a supplement lookup, a
// GetOrCreateLocalStorage walk, and a per-call UseCounter (which fires every
// time for file:// origins). A get+set loop pays that twice per iteration, so we
// cache the resolved area per window (WeakMember key: the entry drops when the
// window dies) and re-resolve only on a miss.
// Aliased so the comma in the template args doesn't confuse the DEFINE_STATIC_
// LOCAL macro (which would read it as an extra macro argument).
using MerseyStorageCache =
    GCedHeapHashMap<WeakMember<LocalDOMWindow>, Member<StorageArea>>;

StorageArea* MerseyResolveLocalStorage(LocalDOMWindow* window) {
  if (!window) {
    return nullptr;
  }
  DEFINE_STATIC_LOCAL(Persistent<MerseyStorageCache>, cache,
                      (MakeGarbageCollected<MerseyStorageCache>()));
  auto it = cache->find(window);
  if (it != cache->end()) {
    return it->value.Get();
  }
  DummyExceptionStateForTesting area_state;
  StorageArea* area = DOMWindowStorage::localStorage(*window, area_state);
  if (!area || area_state.HadException()) {
    return nullptr;
  }
  cache->Set(window, area);
  return area;
}

String MerseyLocalStorageGet(LocalDOMWindow* window, const String& key) {
  StorageArea* area = MerseyResolveLocalStorage(window);
  if (!area) {
    return String();
  }
  DummyExceptionStateForTesting get_state;
  String value = area->getItem(key, get_state);
  return get_state.HadException() ? String() : value;
}

void MerseyLocalStorageSet(LocalDOMWindow* window,
                           const String& key,
                           const String& value) {
  StorageArea* area = MerseyResolveLocalStorage(window);
  if (!area) {
  ...
  DummyExceptionStateForTesting set_state;
  area->setItem(key, value, set_state);
}

// Core->modules hook backing the Mersey engine's Canvas 2D. The 2D context type
// (CanvasRenderingContext2D) is a modules class, so the core runner can't hold
// it — instead it lives here in a persistent registry keyed by a small id, and
// the runner passes that id back for each draw call.
GCedHeapVector<Member<CanvasRenderingContext2D>>& MerseyCanvasRegistry() {
  DEFINE_STATIC_LOCAL(
      Persistent<GCedHeapVector<Member<CanvasRenderingContext2D>>>, registry,
      (MakeGarbageCollected<
          GCedHeapVector<Member<CanvasRenderingContext2D>>>()));
  return *registry;
}

int MerseyCanvasGetContext(HTMLCanvasElement* canvas) {
  if (!canvas) {
    return -1;
  }
  CanvasContextCreationAttributesCore attributes;
  CanvasRenderingContext* context = canvas->GetCanvasRenderingContext(
      canvas->GetExecutionContext(), "2d", attributes);
  if (!context || !context->IsRenderingContext2D()) {
    return -1;
  }
  auto& registry = MerseyCanvasRegistry();
  registry.push_back(static_cast<CanvasRenderingContext2D*>(context));
  return static_cast<int>(registry.size());  // id = index + 1 (>= 1)
}

void MerseyCanvasFillRect(int ctx_id,
                          double x,
                          double y,
                          double w,
                          double h) {
  auto& registry = MerseyCanvasRegistry();
  if (ctx_id >= 1 && static_cast<wtf_size_t>(ctx_id) <= registry.size()) {
    registry[ctx_id - 1]->fillRect(x, y, w, h);
  }
}
...
void ModulesInitializer::Initialize() {
  ...
  // Back the Mersey engine's modules-layer web APIs (localStorage, Canvas 2D)
  // from here.
  {
    MerseyModulesBridge& mersey_bridge = GetMerseyModulesBridge();
    mersey_bridge.local_storage_get = &MerseyLocalStorageGet;
    mersey_bridge.local_storage_set = &MerseyLocalStorageSet;
    mersey_bridge.canvas_get_context = &MerseyCanvasGetContext;
    mersey_bridge.canvas_fill_rect = &MerseyCanvasFillRect;
  }
