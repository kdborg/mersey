...
ScriptLoader::ScriptTypeAtPrepare ScriptLoader::GetScriptTypeAtPrepare(
  ...
  // Mersey fork: the Mersey MIME type routes to the Mersey engine.
  if (EqualIgnoringAsciiCase(type, "text/mersey")) {
    return ScriptTypeAtPrepare::kMersey;
  }
...
PendingScript* ScriptLoader::PrepareScript(
  ...
  switch (GetScriptType()) {
    ...
    case ScriptTypeAtPrepare::kMersey:
      break;
  }
  ...
  if (element_->HasSourceAttribute()) {
    ...
    // Mersey fork: an external Mersey script is loaded and run asynchronously
    // by the engine's own graph loader (below), which never resolves a
    // ClassicPendingScript — so it must not register as render-blocking, or
    // rendering and the load event would block forever.
    if (potentially_render_blocking &&
        GetScriptType() != ScriptTypeAtPrepare::kMersey &&
    ...
    switch (GetScriptType()) {
      ...
      case ScriptTypeAtPrepare::kMersey:
        // Mersey fork: an external Mersey script runs natively via the
        // module-graph loader (fetch entry + imports, scan_imports, run_graph).
        // `src` is the spec as written — the base for relative resolution.
        MerseyScriptRunner::From(element_document)
            .LoadExternalGraph(src.ToString());
        return nullptr;
  ...
  if (!element_->HasSourceAttribute()) {
    ...
    switch (GetScriptType()) {
      ...
      // Mersey fork: an inline Mersey script runs on the Mersey engine, now,
      // the way an inline speculation-rules block is consumed at prepare
      // time. Diagnostics and runtime errors surface on the console through
      // the engine's host table; no pending script is created and V8 is never
      // involved.
      case ScriptTypeAtPrepare::kMersey: {
        // `position` is the parser's location of the script text — the tag's
        // line — which DevTools needs to map document lines to engine lines.
        MerseyScriptRunner::From(element_document)
            .Run(source_text, position.line_.ZeroBasedInt());
        return nullptr;
      }
